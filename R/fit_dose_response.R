#' Fit Dose-Response Curves and Calculate IC50 Values
#'
...
fit_dose_response <- function(data, output_file = NULL, normalize = FALSE, verbose = TRUE, 
  ...
  # Core 3-parameter logistic model (Hill Slope fixo = -1)
  three_param_model <- function(log_inhibitor, Bottom, Top, LogIC50) {
    Bottom + (Top - Bottom) / (1 + 10^(log_inhibitor - LogIC50))
  }
  
  # 4-parameter logistic model para calcular Hill Slope ideal
  ...
  recalculate_dependent_params <- function(params, corrections) {
    ...
    # Recalculate dependent parameters USING CORRECTED VALUES
  ...
  recalculate_ci <- function(original_ci, corrections) {
    ...
    # Mark CIs as NA for corrected parameters
    ...
    # If Bottom or Top are corrected, LogIC50 and IC50 CIs are also affected
  ...
  # Robust nonlinear fitting with multiple strategies (Hill Slope fixo = -1)
  try_robust_fit <- function(df_clean, min_resp, max_resp, approx_ic50) {
    ...
    # Try all strategies with default control (3-parameter model)
    for (start_vals in start_strategies) {
      fit <- tryCatch({
        stats::nls(
          response ~ three_param_model(log_inhibitor, Bottom, Top, LogIC50),
  ...
  calculate_ideal_hill_slope <- function(df_clean, initial_params) {
    ...
    tryCatch({
      # Use parameters from 3-parameter fit as starting values
      start_vals <- list(
        Bottom = initial_params[1],
        Top = initial_params[2],
        LogIC50 = initial_params[3],
        HillSlope = -1  # Start with Hill Slope = -1
      )
      ...
      # Extract Hill Slope value
      hill_slope <- stats::coef(fit_4param)["HillSlope"]
      return(round(as.numeric(hill_slope), 3))
      
    }, error = function(e) {
      # If 4-parameter fit fails, return NA
      return(NA)
    })
  }
  ...
  analyze_single_pair <- function(pair_data, comp_name) {
    ...
    # Calculate ideal Hill Slope (4-parameter model)
  ...
  # Create summary table - REMOVIDA coluna Threshold_Applied
  ...
  # Print summary statistics - ATUALIZADO com aviso dos compostos afetados
  if (verbose) {
